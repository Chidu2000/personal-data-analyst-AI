...
from dataframe_detect import suggest_prompts
from run_code import run_code, ask_llm
from llm_to_code_convertion import prompt_to_code
import pandas as pd
...
suggestions = suggest_prompts(df) # 2
...
if st.button("Run analysis"):
    with st.spinner("Running..."):
        ...
        code = prompt_to_code(final_prompt, df)  # 3
        if code:
            res = run_code(df, code)  # 4
        else:
            ...
            if use_llm:
                ...
                llm_out = ask_llm(raw, model=llm_model)  # 5
